    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            try {
                ...
                alert("E2: Invalid JSON response.");
                return false;
            }

            // Check if the expected properties are present in the response
            if (details && details.hasOwnProperty("status") && details.hasOwnProperty("message")) {
                if (details.status === "200") {
                    alert(details.message);
                    window.location.replace("login.php");
                } else {
                ...
                console.error("Missing expected properties in the JSON response.");
                alert("E3: Invalid response format.");
            }
        },
        error: function() {
            console.error("E1: Signup Error.");
            alert("E1: Signup Error.");
            return false;
        }
    });
